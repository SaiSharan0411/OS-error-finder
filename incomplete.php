...
if(isset($_POST['completed'])) {
    $tname=$_POST['tname'];
    $query="UPDATE list SET status='C' WHERE uname='$uname' AND tname='$tname'";
    mysqli_query($con,$query);
}
$query="SELECT tname,date,time,status FROM list WHERE uname='$uname' AND status='I' ORDER BY date,time";
...
while($task=mysqli_fetch_assoc($result)) {
    ...
    echo'<td><center>'.$task['status'].'</center></td>';
    echo'<td>
